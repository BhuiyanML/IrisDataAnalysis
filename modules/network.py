...
class SharedAtrousConv2d(nn.Module):
    """
    A class for shared atrous convolutional layers with two parallel convolutions.
    """
    ...
    def forward(self, x):
        x1 = F.conv2d(x, self.weights, stride=1, padding='same', bias=self.bias1)
        x2 = F.conv2d(x, self.weights, stride=1, padding='same', dilation=2, bias=self.bias2)
        return torch.cat([x1, x2], 1)


class SharedAtrousResBlock(nn.Module):
    """
    A class for shared atrous residual blocks.
    """
...
class Resize(nn.Module):
    """
    A class for resizing tensors using interpolation.
    """
...
class NestedSharedAtrousResUNet(nn.Module):
    """
    A nested shared atrous residual U-Net model for segmentation tasks.
    """
...
class FCLayerConvNext(nn.Module):
    def __init__(self, in_h=8, in_w=10, out_n=6):
        super(FCLayerConvNext, self).__init__()
        self.in_h = in_h
        self.in_w = in_w
        self.out_n = out_n
...
class ConvConvNext(nn.Module):
    def __init__(self, in_channels=768, out_n=6):
        super(ConvConvNext, self).__init__()
